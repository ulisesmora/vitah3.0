...
function firebaseInit() {
    ...
    //        $scope.videosModulos = [];
            ...
            function addVideos(moduloId, contador) {
                ...
                db.collection("modulos").doc(moduloId).collection("videos").where("videoActivo", "!=", 3).get().then(function (querySnapshot) {
                    querySnapshot.forEach(function (doc) {
                        let like = doc.data().likes != undefined ? doc.data().likes : 0;
                        let dislike =doc.data().dislikes != undefined ? doc.data().dislikes : 0;
                        var video = {
                            nombre: doc.data().nombre,
                       ...
                       // likestotales = doc.data().likes + likestotales;
                       // dislikestotales = doc.data().dislikes + dislikestotales;
                   ...
                   // $scope.addModulo[contador - 1].push(totalLikes);
                   // $scope.addModulo[contador - 1].push(DistotalLikes);
                }).catch(function (error) {
                    console.log("Error getting documents: ", error);
                });
            }
    ...
    //            XLSX.utils.sheet_add_json(ws, json4, {header: ["A", "B", "C", "D", "E", "F", "G"], skipHeader: true, origin: -1});
                ...
                console.log(ws,"de donde sale esto")
    //            if (!ws['!cols'][1])
    //                ws['!cols'][1] = {};
                for (var i = 0; i < $scope.addModulo.length; i++) {
                    var n = i;
                    var recorrer = 0 + n + n;
                    console.log($scope.addModulo[i])
                    XLSX.utils.sheet_add_json(ws, $scope.addModulo[i], {skipHeader: true, origin: {r: 2, c: recorrer + i}});
    //            ws['!cols'][1].hidden = true;
                    ws["!cols"].push({wpx: 300, s: {font: {bold: true}}}, {wpx: 100}, {wpx: 50}, );
                }
                workbookModulos.Sheets["MODULOS"] = ws;
                console.log(ws,"wes anderson");
