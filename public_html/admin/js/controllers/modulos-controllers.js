...
//        $scope.video = {files:""};
        ...
        document.getElementById("formulario-video").style.display = "none";
        ...
        function agregarImagenVideo(imagen, direccionBD, moduloId, videoId) {
            var file = imagen;
            if (file === undefined) {
                imageURL = "";
                direccionBD.set({
                ...
                const task = storage.child("modulos/" + moduloId + "/perfil/" + moduloId + "/videos/" + videoId).put(file, metadata);
                task.then(snapshot => snapshot.ref.getDownloadURL()).then(url => {
//                    const image = document.querySelector('#image');
                    imageURL = url;
                    direccionBD.set({
        ...
        function agregarEjercicios(videos, index, videoId, moduloId) {
            for (let i = 0; i < videos.ejercicios.length; i++) {
                    db.collection('modulos').doc(moduloId).collection("videos").doc(videoId).collection("ejercicios").doc('"'+i+'"').set({
                        instrucciones: videos.ejercicios[i].instrucciones,
                        preguntas: videos.ejercicios[i].preguntas
                    }).then(function (docVideoRef) {
                        $scope.$digest();
                    }).catch(function (error) {
                        console.log("Error getting cached document:", error);
        ...
        $scope.crearFormulario = function () {
            document.getElementById("datos-video").style.display = "none";
            document.getElementById("formulario-video").style.display = "block";
        };
        ...
        $scope.guardarPregunta = function (pregunta) {
            ...
            } else if (((evaluarCampo(pregunta, 'pregunta')) === true)) {
                $scope.preguntas.push(angular.copy(pregunta));
                $scope.ejercicio.preguntas = $scope.preguntas;
//                var index = $scope.videosAgregados.length - 1;
//                $scope.videosAgregados[index].files = files;
//                $scope.videosAgregados[index].thumbnail = file;
                $scope.pregunta = undefined;
//                $scope.files = undefined;
            }
        };
        $scope.guardarEjercicio = function (ejercicio) {
            $scope.ejercicios.push(angular.copy(ejercicio));
            $scope.ejercicio = undefined;
            $scope.preguntas = [];
        ...
        $scope.agregarVideo = function (video, files) {
            var file = document.querySelector('#videoImagen').files[0];
            if (video === undefined) {
                new Noty({
                    type: 'error',
                    layout: 'center',
                ...
                var index = $scope.videosAgregados.length - 1;
                $scope.videosAgregados[index].ejercicios = $scope.ejercicios;
                $scope.videosAgregados[index].files = files;
                $scope.videosAgregados[index].thumbnail = file;
        ...
        $scope.eliminarVideo = function (pregunta) {
            for (var i = 0; i < $scope.preguntas.length; i++) {
                if (pregunta === $scope.preguntas[i]) {
            ...
            if ((evaluarCampo(modulo.nombreModulo, 'Nombre del módulo') && evaluarCampo(videos, 'la lista de videos')) === true) {
                db.collection('modulos').add({
                    ...
                    for (var k = 0; k < videos.length; k++) {
                        ...
                        if (videos[k].preguntas === undefined) {
                            videos[k].preguntas = "";
                        }
                        imagenVideo = videos[k].thumbnail;
                        detailModuloRef.collection('videos').add({
                            ...
                            videoActivo: 1,
                            contadorVistas: 0
//                            preguntas: videos[k].preguntas
                        }).then(function (docVideoRef) {
                            console.log(index);
                            var detailVideoRef = db.collection('modulos').doc(docDetailRef.id).collection("videos").doc(docVideoRef.id);
                            agregarImagenVideo(imagenVideo, detailVideoRef, docDetailRef.id, docVideoRef.id);
                            setTimeout(agregarEjercicios, 1000, videos[index], index, docVideoRef.id, docDetailRef.id);
                            $scope.$digest();
                        }).catch(function (error) {
                            console.log("Error getting cached document:", error);
                        });
//                        console.log($scope.videoId);
//                        setTimeout(agregarEjercicios, 1000, videos[k], index, $scope.videoId, docDetailRef.id);
//                        agregarEjercicios(videos[k], index, videoId, docDetailRef.id);
                    }
...
angular.module('app').controller("EditarModuloCtrl", [
    ...
    function ($scope, $state, $stateParams) {
        ...
        function agregarImagenVideo(idImagen, direccionBD, moduloId, videoId) {
            var file = document.querySelector(idImagen).files[0];
            if (file !== undefined) {
...
//                    const image = document.querySelector('#image');
                    imageURL = url;
                    db.collection('modulos').doc(moduloId).collection("videos").doc(videoId).set({
                        imageURI: imageURL
                    }, {merge: true});
                    $scope.$digest();
                }).catch(function (error) {
...
////                if ((videoDB.nombre === arrayVideos[i].nombreVideo) && (videoDB.link === arrayVideos[i].linkVideo)) {
        ...
        $scope.editarVideo = function (video) {
            ...
            const image = document.querySelector('#imageVideo');
            image.src = $scope.video.imageURI;
            urlSinImagen = $scope.video.imageURI;
        ...
        $scope.activeVideo = async function(video){
            
            if(video.videoActivo == 1){
                console.log("aquí");
              await  db.collection("modulos").doc(id).collection("videos").doc(video.id).update({
                    videoActivo: 2
                })
            }else if(video.videoActivo == 2){
                console.log("herree");
                await  db.collection("modulos").doc(id).collection("videos").doc(video.id).update({
                    videoActivo: 1
                })
            ...
            if ($scope.video.id === video.id) {
                if ((evaluarCampo(video.nombre, "el nombre del video") && evaluarCampo(video.link, "link del video")) === true) {
                    ...
                    db.collection("modulos").doc(id).collection("videos").doc(video.id).update({
                        ...
                        agregarImagenVideo("#videoImagen", detailVideoRef, id, video.id);
//                        var n = new Noty({
