...
//                            if ($scope.empleadoSesion.status !== 1) {
////                                $state.go('main.preregistro');
//                            }
                            $scope.$digest();
...
//                            $scope.diezVideos[i].see = true;
        ...
        async function likeAVideoByUser(videoVistoId, like) {
            const result = await db.collection("clientesPrivados").doc($scope.empleadoSesion.empleadoid).collection("videosVistos").doc(videoVistoId).get();
            console.log(result,"ek resukt");
            db.collection("clientesPrivados").doc($scope.empleadoSesion.empleadoid).collection("videosVistos").doc(videoVistoId).set({
                like: like
            }, {merge: true});
        }
