            ...
            auth.onAuthStateChanged(user => {
                if (user) {
                    user.getIdTokenResult().then(idTokenResult => {
                        ...
                        db.collection('clientesPrivados').where("id", "==", empleadoAuthId).get().then((snapshot) => {
...
//            var fechaValida = comprobarDiayMes(empleado.diaNacimiento, empleado.mesNacimiento, empleado.anioNacimiento);
            //            if (fechaValida === true) {
            if ((evaluarCampo(empleado.nombre, "nombre") && evaluarCampo(empleado.primerApellido, "primer apellido") && evaluarCampo(empleado.segundoApellido, "segundo apellido") 
                    && evaluarCampo(empleado.telefono, "teléfono")) === true) {
//                var fechaNacimiento = empleado.mesNacimiento + "/" + empleado.diaNacimiento + "/" + empleado.anioNacimiento;
                db.collection("clientesPrivados").doc(empleado.empleadoId).update({
