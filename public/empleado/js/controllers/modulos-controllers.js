    ...
    function ($scope, $state, $stateParams, $rootScope, $sce) {
        ...
        function getVideosVistosByUser() {
            auth.onAuthStateChanged(user => {
                if (user) {
                    user.getIdTokenResult().then(idTokenResult => {
                        empleadoAuthId = idTokenResult.claims.user_id;
                        ...
                        db.collection('empresas').doc(empresaId).collection('empleados').where("id", "==", empleadoAuthId).get().then((snapshot) => {
                            ...
                            db.collection("empresas").doc(empresaId).collection("empleados").doc($scope.id).collection("videosVistos").get().then(function (querySnapshot) {
...
//                        $rootScope.linkVideo = $sce.trustAsResourceUrl("https://player.vimeo.com/video/" + video[3]);
        ...
        function likeAVideoByUser(videoVistoId, like) {
            db.collection("empresas").doc(empresaId).collection("empleados").doc($scope.id).collection("videosVistos").doc(videoVistoId).set({
                like: like
            }, {merge: true});
        }
